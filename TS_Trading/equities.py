...
class equity:
    ...
    def __init__(self, frame, x, y):
        ...
        self.buyMkt = (self.buyMkt[0] + int(self.x), self.buyMkt[1] + int(self.y))
        if self.buyMkt:
            self.positionArearect = (int(self.buyMkt[0]) + 50, int(self.buyMkt[1]) - 40)
            self.profitLossArearect = (int(self.buyMkt[0]) + 100, int(self.buyMkt[1]) - 20)
            self.cancelExit = (self.buyMkt[0] + 110, self.buyMkt[1])
            self.tickStrikeArearect = (int(self.cancelExit[0]) + 1, int(self.cancelExit[1]) + 60)
        self.sellMkt = firstFind(self.frame, self.sellMkt, self.sellMktImg)
        self.sellMkt = (self.sellMkt[0] + int(self.x), self.sellMkt[1] + int(self.y))
    ...
    def getPosition(self, frame):
        ...
        redPixels = np.argwhere(cv2.inRange(self.positionArea, (0, 0, 200), (100, 100, 255)))
        if redPixels.size >= 1:
